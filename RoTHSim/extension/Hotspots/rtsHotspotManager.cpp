#include "rtsHotspotManager.h"

rtsHotspotManager::rtsHotspotManager(cGenericObject* a_parent)
{
...
rtsHotspotManager::~rtsHotspotManager()
{
	deallocate();
}

void rtsHotspotManager::addTargetGroup(double a_radius, int a_trueTarget, int a_bldgId, double a_startTime, double a_endTime, vector<cVector3d> a_targetPos, int a_numTargets, int a_groupId)
{
	rtsHotspotGroup* targetGroup = new rtsHotspotGroup(a_groupId);
	for (int i = 0; i < a_numTargets; i++)
	{
		// create new target
		rtsHotspot* target = new rtsHotspot(a_radius, a_startTime, a_endTime);
...
void rtsHotspotManager::setTargetGroups(vector<rtsHotspotGroup*> a_targetGroups)
{
...
void rtsHotspotManager::updateTargetStates(vector<cVector3d> a_agentPositions)
{
...
vector<cVector3d> rtsHotspotManager::getActiveTargetPositions()
{
...
vector<rtsHotspotGroup*> rtsHotspotManager::getActiveTargetGroups()
{
	vector<rtsHotspotGroup*> activeGroups;
	int numGroups = m_targetGroups.size();
	for (int i = 0; i < numGroups; i++)
	{
...
void rtsHotspotManager::updateActiveTargetStates(double a_simTime, int a_currentBldgId)
{
	m_numActiveTargets = 0;
	int numGroups = m_targetGroups.size();
	for (int i = 0; i < numGroups; i++)
...
bool rtsHotspotManager::writeTargetFoundData(string a_filename)
{
...
void rtsHotspotManager::setShowTargets(bool a_show)
{
	int numTargetGroups = m_targetGroups.size();
	for (int i = 0; i < numTargetGroups; i++)
	{
...
void rtsHotspotManager::deallocate()
{
	// delete all targets
	vector<rtsHotspotGroup*>::iterator it;
	for (it = m_targetGroups.begin(); it < m_targetGroups.end(); it++)
	{
		rtsHotspotGroup* nextTargetGroup = (*it);
