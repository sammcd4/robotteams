
//---------------------------------------------------------------------------
#include "rtsClay.h"
//---------------------------------------------------------------------------
typedef GLUquadric GLUquadricObj;
//---------------------------------------------------------------------------

//===========================================================================
/*!
    Constructor of rtsClay.

    \fn     rtsClay::rtsClay()
    \param  a_world
	\param  a_robotPositions
*/
//===========================================================================
rtsClay::rtsClay(cWorld* a_world, vector<cVector3d> a_robotPositions, double a_toolRadius, double a_toolWorkspaceScaleFactor, double a_maxStiffness, bool a_useBuildingBounds)
{
	// set parent world
	m_world = a_world;

	// create deformable world
	m_defWorld = new rtsClayWorld();
	m_world->addChild(m_defWorld);
	
	// set robot positions
	m_robotPositions = a_robotPositions;

	// set tool radius
	m_toolRadius = a_toolRadius;

	// set workspace scale factor
	m_toolWorkspaceScaleFactor = a_toolWorkspaceScaleFactor;

	// set max stiffness
	m_maxNodeStiffness = a_maxStiffness;

	// initialize surround parameters
	initSurroundParams();

	// create a deformable playdoh mesh for surround
	m_surroundMesh = new rtsClayMesh(m_world);

	// add surround mesh to deformable world meshes
	m_defWorld->m_gelMeshes.push_back(m_surroundMesh);

	// force surround mode to initialize surround
	setMode(1);

	// create surround
	initSurround();

	// create a deformable playdoh mesh for travel
	m_travelMesh = new rtsClayMesh(m_world);

	// add travel mesh to deformable world meshes
	m_defWorld->m_gelMeshes.push_back(m_travelMesh);

	// create static surround mesh
	m_surroundStaticResizeMesh = new rtsClayMesh(m_world);
	m_defWorld->m_gelMeshes.push_back(m_surroundStaticResizeMesh);

	// set default mode
	m_defaultMode = -1;
	//setMode(m_defaultMode);

	// travel links enable
	m_useTravelLinks = true;

	// reset travel
	m_resetTravel = false;
	m_resetTravelCounter = 0;

	// set travel node color
	m_travelNodeColor = cColorf(0.5, 0.0, 0.0, 1.0f);

	// set surround node color
	m_surroundNodeColor = cColorf(
		0 / 256.0,
		85 / 256.0,
		256 / 256.0, 1);

	// set resize surround
	m_resizeSurround = false;

	// contact state
	m_contacted = false;

	// previous contact state
	m_prevContacted = false;

	// use building bounds
	m_useBuildingBounds = a_useBuildingBounds;

	// set spacing factor
	m_spacingTol = 0.20;

	// transition timeout
	m_transitionTimeout = 1.0;
	m_transitionTimer.setTimeoutPeriodSeconds(m_transitionTimeout);

	//// set transition force factor time interval
	//m_transitionForceFactorTimeInterval = m_transitionTimeout / 100;

	// transition
	m_transition = false;
	m_prevTransition = false;

	// initialize forces
	m_toolForce = cVector3d(0, 0, 0);
	m_prevToolForce = cVector3d(0, 0, 0);
	m_playdohForce = cVector3d(0, 0, 0);
	m_prevPlaydohForce = cVector3d(0, 0, 0);
	m_transitionForce = cVector3d(0, 0, 0);

	// center of mass initialization
	m_centerOfMass = cVector3d(0, 0, 0);
	int numRobots = m_robotPositions.size();
	for (int i = 0; i < numRobots; i++)
		m_centerOfMass += m_robotPositions[i];
	m_centerOfMass.div(numRobots);

	// initialize spacing timer
	m_spacingClock.setTimeoutPeriodSeconds(0.1);
	m_spacingClock.reset();
	m_spacingClock.start();
}

//---------------------------------------------------------------------------

rtsClay::~rtsClay()
{

}

//---------------------------------------------------------------------------

void rtsClay::initPlaydoh()
{
	// hide visual and haptics
	setShowEnabled(false, true);
	setHapticEnabled(false, true);

	// force travel mode to initialize travel
	setMode(0);

	// create travel
	initTravel();

	// set initial previous convex hull mapping
	m_prevConvexHullMapping = m_convexHullMapping;
}

//---------------------------------------------------------------------------

void rtsClay::setShowEnabled(bool a_showEnabled, bool a_affectChildren) 
{ 
	// set member variable
	m_showEnabled = a_showEnabled;

	if (m_surroundMode)
		m_surroundMesh->setShowEnabled(a_showEnabled, a_affectChildren);
	else if (m_travelMode)
		m_travelMesh->setShowEnabled(a_showEnabled, a_affectChildren);

}

//---------------------------------------------------------------------------

bool rtsClay::getShowEnabled()
{
	if (m_surroundMode)
		return m_surroundMesh->getShowEnabled();
	else if (m_travelMode)
		return m_travelMesh->getShowEnabled();
	else return false;
}

//---------------------------------------------------------------------------

void rtsClay::setHapticEnabled(bool a_hapticEnabled, bool a_affectChildren)
{
	m_affectHapticChildren = a_affectChildren;
	m_hapticEnabled = a_hapticEnabled;
}

//---------------------------------------------------------------------------

void rtsClay::setStiffness(double a_stiffness)
{
	rtsClayNode* temp;
	vector<rtsClayNode*>::iterator iter;
	for (iter = m_surroundSpheres.begin(); iter != m_surroundSpheres.end(); iter++)
	{
		// dereference iterator
		temp = *iter;

		// set stiffness for the sphere
		temp->setStiffness(a_stiffness);
	}
}

//---------------------------------------------------------------------------

void rtsClay::updateRobotPositions(vector<cVector3d> a_robotPositions)
{
	if (!m_standbyMode && !m_resizeSurround)
	{
		// update the vector storing robot positions
		m_robotPositions = a_robotPositions;

		rtsClayNode* temp;
		cVector3d pos;
		vector<rtsClayNode*>::iterator iter;
		vector<cVector3d>::iterator iterPos;

		vector<rtsClayNode*> a_robotSpheres;
		if (m_surroundMode)
		{
			// beginning robot position
			iterPos = m_robotPositions.begin();

			a_robotSpheres = m_robotSurroundSpheres;
		}
		else if (m_travelMode)
		{
			// beginning robot position
			iterPos = m_convexHullRobotPositions.begin();

			a_robotSpheres = m_robotTravelSpheres;
		}


		// update the physical position of robot-containing nodes
		for (iter = a_robotSpheres.begin(); iter != a_robotSpheres.end(); iter++)
		{
			// dereference iterators
			temp = *iter;
			pos = *iterPos;

			// set position of node
			temp->m_nextPos = pos;
			temp->applyNextPose();
			//temp->m_pos = pos;

			// increment vector iterator
			iterPos++;
		}

		// assign member variables to new positions
		if (m_travelMode)
		{
			m_robotTravelSpheres = a_robotSpheres;
		}
		else if (m_surroundMode)
		{
			m_robotSurroundSpheres = a_robotSpheres;
		}

	}
}

//---------------------------------------------------------------------------

void rtsClay::setConvexHull(vector<cVector3d> a_convexHull)
{
	// set convex hull
	m_convexHullRobotPositions = a_convexHull;
}

//---------------------------------------------------------------------------

void rtsClay::setConvexHullMapping(vector<int> a_convexHullMapping)
{
	// set member variable
	m_convexHullMapping = a_convexHullMapping;

	//m_resetTravel = false;
	if (m_convexHullMapping != m_prevConvexHullMapping)
	{
		// call for a reset in the travel
		m_resetTravel = true;

		// start transition
		startTransition();
	}

	// store previous convex hull mapping for next iteration
	m_prevConvexHullMapping = m_convexHullMapping;
}

//---------------------------------------------------------------------------

cVector3d rtsClay::computeForce(const cVector3d& a_cursor, double a_cursorRadius, int a_closestIndex, double a_modFactor, bool a_manip, bool a_manipSpacing)
{
	// zero force
	cVector3d force;
	force.zero();
	//a_manip = false;
	//a_manipSpacing = true;
	// set previous playdoh force
	m_prevPlaydohForce = m_playdohForce;

	// set previous tool force
	m_prevToolForce = m_toolForce;

	// set previous contact state
	m_prevContacted = m_contacted;

	// reset contact state of playdoh
	m_contacted = false;

	// object pointers
	rtsClayNode* current;
	rtsClayNode* next;
	rtsClayNode* prev;
	rtsClayLink* currentLink;
	rtsClayLink* nextLink;
	//rtsClayLink* prevLink;

	// iterators
	vector<rtsClayNode*>::iterator iter;
	vector<rtsClayNode*>::iterator iterNext;
	vector<rtsClayNode*>::iterator iterPrev;
	vector<rtsClayNode*>::iterator iterPrevRobotNode;
	vector<rtsClayNode*>::iterator iterCurrentRobotNode;
	rtsClayNode* prevRobotNode;
	rtsClayNode* currentRobotNode;
	int currentRobotNodeIndex = 0;
	int prevRobotNodeIndex = 0;

	if (!m_robotSurroundSpheres.empty())
	{
		prevRobotNode = m_robotSurroundSpheres[currentRobotNodeIndex];
		currentRobotNode = prevRobotNode;
	}
	double totalInnerDistSum = 0;
	double totalSpringForceSum = 0;

	list<rtsClayLink*>::iterator iterLink;
	list<rtsClayLink*>::iterator iterLinkNext;
	list<rtsClayLink*>::iterator iterLinkPrev;
	list<rtsClayNode*>::iterator meshNode;
	list<rtsClayNode*>::iterator prevMeshNode;

	// set local spheres and mesh variables
	vector<rtsClayNode*> a_spheres;
	rtsClayMesh* a_mesh;
	if (m_surroundMode)
	{
		a_spheres = m_surroundSpheres;
		a_mesh = m_surroundMesh;
	}
	else if (m_travelMode)
	{
		a_spheres = m_travelSpheres;
		a_mesh = m_travelMesh;
	}

	// check spacing timeout so that nodes aren't moving as fast as computeForce is called (1kHz)
	bool spacingTimeout = false;
	if (m_spacingClock.timeoutOccurred())
	{
		m_spacingClock.stop();
		spacingTimeout = true;
	}

	// winding number variable
	int wn = 0;
	double penetrationDist = (a_cursor.z() - a_cursorRadius) - m_travelHeight;

	// if resizing is being done, then skip this function.
	// no force computation is needed to resize
	if (m_resizeSurround)
	{
		// resize surround instead of calculate force
		resizeSurround();
	}
	else if (m_resetTravel)
	{
		// need to set force to previously calculated tool force to avoid a zero force between iterations
		force = m_prevToolForce;

		// set current playdoh force
		m_playdohForce = -1 * force;

		// set current tool force
		m_toolForce = force;

		// reset variable
		m_resetTravelCounter++;
		if (m_resetTravelCounter > 1)
		{
			m_resetTravel = false;
			m_resetTravelCounter = 0;
		}
	}
	// compute force, if mesh exists
	else if(!a_spheres.empty())
	{
		// starting iterator values
		iterPrev = a_spheres.end();
		--iterPrev;
		prev = *iterPrev;

		meshNode = a_mesh->m_nodes.begin();
		prevMeshNode = a_mesh->m_nodes.end();
		--prevMeshNode;

		if(m_surroundMode || (m_travelMode && m_useTravelLinks))
		{
			iterLink = a_mesh->m_links.begin();
			iterLinkPrev = a_mesh->m_links.end();
			--iterLinkPrev;
		}

		double tempPerimeter;
		if (m_surroundMode)
		{
			// temporary perimeter
			tempPerimeter = 0;
		}

		// set a flag that triggers once a contacted sphere is reached
		bool manipFlag = false;
		int numFollowingManipRobots = 0;

		cVector3d xyzBar;
		xyzBar.zero();
		double totalMass = 0;
		bool externalForceApplied = false;

		// inner node distances
		bool innerDistStart = false;
		double innerDistSum = 0;

		// update spacing factors
		if (a_manipSpacing)
			updateSpacingFactors(a_closestIndex, a_modFactor);

		for (iter = a_spheres.begin(); iter != a_spheres.end(); iter++)
		{
			// set current sphere
			current = *iter;

			// set next sphere
			iterNext = iter;
			if ((iterNext != a_spheres.end()) && ((++iterNext) == a_spheres.end()))
			{
				next = *(a_spheres.begin());
			}
			else
			{
				next = *iterNext;
			}

			// set prev sphere
			if (iter == a_spheres.begin())
			{
				prev = a_spheres.back();
			}

			// set link iterators
			if (m_surroundMode || (m_travelMode && m_useTravelLinks))
			{
				// set current link
				currentLink = *iterLink;

				// set next link
				iterLinkNext = iterLink;
				if ((iterLinkNext != a_mesh->m_links.end()) && ((++iterLinkNext) == a_mesh->m_links.end()))
				{
					nextLink = *(a_mesh->m_links.begin());
				}
				else
				{
					nextLink = *iterLinkNext;
				}
			}

			// calculate mass times x and y
			double mass = current->m_mass;
			xyzBar.x(xyzBar.x() + mass * current->m_pos.x());
			xyzBar.y(xyzBar.y() + mass * current->m_pos.y());
			xyzBar.z(xyzBar.z() + mass * current->m_pos.z());
			totalMass += mass;

			// current and next positions
			cVector3d currentPos = current->m_pos;
			cVector3d nextPos = next->m_pos;

			// calculate playdoh bounding box
			//--------------------------------
			if (iter == a_spheres.begin())
			{
				m_boundingBoxPos = currentPos;
				m_boundingBoxNeg = currentPos;
			}
			if ((currentPos.x() - current->m_radius) < m_boundingBoxNeg.x())
			{
				m_boundingBoxNeg.x(currentPos.x() - current->m_radius);
			}
			if ((currentPos.y() - current->m_radius) < m_boundingBoxNeg.y())
			{
				m_boundingBoxNeg.y(currentPos.y() - current->m_radius);
			}
			if ((currentPos.z() - current->m_radius) < m_boundingBoxNeg.z())
			{
				m_boundingBoxNeg.z((currentPos.z() - current->m_radius));
			}
			if ((currentPos.x() + current->m_radius) > m_boundingBoxPos.x())
			{
				m_boundingBoxPos.x(currentPos.x() + current->m_radius);
			}
			if ((currentPos.y() + current->m_radius) > m_boundingBoxPos.y())
			{
				m_boundingBoxPos.y(currentPos.y() + current->m_radius);
			}
			if ((currentPos.z() + current->m_radius) > m_boundingBoxPos.z())
			{
				m_boundingBoxPos.z((currentPos.z() + current->m_radius));
			}
			//--------------------------------

			// calculate change in perimeter, and if needed, add spheres in the list
			if (m_surroundMode)
			{
				// calculate addition to perimeter
				cVector3d diff = nextPos - currentPos;
				tempPerimeter = tempPerimeter + diff.length();
			}

			// Adjust node stiffness based on exploration or manipulation
			if (a_manip)
			{
				current->m_stiffness = m_surroundNodeStiffness;
			}
			else current->m_stiffness = m_surroundNodeStiffness / 2;

			// compute external force on node
			cVector3d nodePos = current->m_pos;
			double nodeRadius = current->m_radius;
			double nodeStiffness = current->m_stiffness;
			cVector3d f = computeCylNodeForce(a_cursor, a_cursorRadius, nodePos, nodeRadius, nodeStiffness);
			cVector3d tmpfrc = cNegate(f);
			if (tmpfrc.length() > 0)
			{
				if (!externalForceApplied)
				{
					externalForceApplied = true;
				}

				// set contact state of node
				current->m_contacted = true;

				// set contact state of playdoh
				if (!m_contacted)
				{
					m_contacted = true;
				}
			}
			else
			{
				current->m_contacted = false;
			}

			// update spacing between robot nodes (only if manipulating playdoh)
			if (m_surroundMode && (a_manipSpacing && spacingTimeout))
			{
				bool rnFlag = false;
				if (current->m_atRobot && !current->m_newAtRobot)
				{
					if (!innerDistStart)
					{
						innerDistStart = true;
						iterPrevRobotNode = iter;
						currentRobotNodeIndex = 0;
					}
					else
					{
						// set current robot iterator
						iterCurrentRobotNode = iter;
						currentRobotNodeIndex++;

						// compare node distance to equilibrium distance
						if (innerDistSum > (1.0 + m_spacingTol) * m_uniformRobotSpacing * m_spacingFactors[currentRobotNodeIndex])
						{
							// move previous and current robot nodes closer to each other, if possible
							if (iterPrevRobotNode != a_spheres.begin())
							{
								//rtsClayNode* node = *(iterPrevRobotNode+1);
								//if (!node->m_atRobot)
								//{
								//	m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = false;
								//	m_robotSurroundSpheres[prevRobotNodeIndex] = *(++iterPrevRobotNode);
								//	m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = true;
								//	iterPrevRobotNode++;
								//}
							}

							if (!prev->m_atRobot)
							{
								m_robotSurroundSpheres[currentRobotNodeIndex]->m_atRobot = false;
								prev->m_atRobot = true;
								m_robotSurroundSpheres[currentRobotNodeIndex] = prev;
								iterCurrentRobotNode--;
							}
						}
						else if (innerDistSum < (1.0 - m_spacingTol) * m_uniformRobotSpacing * m_spacingFactors[currentRobotNodeIndex])
						{
							if (!next->m_atRobot)
							{
								m_robotSurroundSpheres[currentRobotNodeIndex]->m_atRobot = false;
								next->m_atRobot = true;
								next->m_newAtRobot = true; // so this node is skipped on next iteration
								m_robotSurroundSpheres[currentRobotNodeIndex] = next;
								iterCurrentRobotNode++;
							}

							// move previous and current robot nodes away from each other
							//if (iterPrevRobotNode != a_spheres.begin())
							//{
							//	rtsClayNode* node = *(iterPrevRobotNode-1);
							//	if (!node->m_atRobot)
							//	{
							//		m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = false;
							//		m_robotSurroundSpheres[prevRobotNodeIndex] = node;
							//		m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = true;
							//		iterPrevRobotNode--;
							//	}
							//}
						}

						// set previous robot iterator
						iterPrevRobotNode = iterCurrentRobotNode;
						prevRobotNodeIndex++;

						rnFlag = true;
					}

					innerDistSum += cDistance(prev->m_pos, current->m_pos);
				}
				// if no more robot nodes because reach end of list
				else if (next == *(a_spheres.begin()))
				{
					// compare node distance to average node distance
					if (innerDistSum > (1.0 + m_spacingTol) * m_uniformRobotSpacing * m_spacingFactors[currentRobotNodeIndex])
					{
						rtsClayNode* node = *(iterPrevRobotNode+1);
						if (!node->m_atRobot)
						{
							// just move previous robot node closer
							m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = false;
							m_robotSurroundSpheres[prevRobotNodeIndex] = node;
							m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = true;
							iterPrevRobotNode++;
						}
						//cout << "end list spacing greater" << endl;
					}
					else if (innerDistSum < (1.0 - m_spacingTol) * m_uniformRobotSpacing * m_spacingFactors[currentRobotNodeIndex])
					{
						rtsClayNode* node = *(iterPrevRobotNode-1);
						if (!node->m_atRobot)
						{
							// just move previous robot node away
							m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = false;
							m_robotSurroundSpheres[prevRobotNodeIndex] = node;
							m_robotSurroundSpheres[prevRobotNodeIndex]->m_atRobot = true;
							iterPrevRobotNode--;
						}
					}

					innerDistSum += cDistance(prev->m_pos, current->m_pos);
					rnFlag = true;
				}
				else
				{
					if (current->m_newAtRobot)
					{
						current->m_newAtRobot = false;
					}
					innerDistSum += cDistance(prev->m_pos, current->m_pos);
				}

				if (rnFlag)
				{
					totalInnerDistSum += innerDistSum;

					// reset inner node distance sum
					innerDistSum = 0;

					rnFlag = false;
				}
			}

			// keep playdoh static when manipulating spacing
			if (a_manipSpacing || (!a_manip && m_contacted))
			{
				current->m_fixed = true;
				tmpfrc.zero();
			}
			else
			{
				current->m_fixed = false;
			}

			// check if node is moving inside building
			if (m_surroundMode && m_useBuildingBounds)
			{
				int numBounds = m_buildingBounds.size();
				bool insideBounds = false;
				for (int i = 0; i < numBounds; i++)
				{
					if (m_buildingBounds[i].isInside(current->m_pos))
					{
						insideBounds = true;
						break;
					}
				}

				if (insideBounds)
				{
					// if previously not inside bldg
					if (!current->m_insideBldg)
					{
						// store entry vector
						current->m_entryVec = cNormalize(current->m_pos - current->m_prevPos);
						current->m_insideBldg = true;
					}

					// apply small force to computed force away from building
					cVector3d insideF = 0.0001*m_toolWorkspaceScaleFactor*(-current->m_entryVec);
					tmpfrc.add(insideF);
				}
				else current->m_insideBldg = false;
			}

			// set external force on current node
			tmpfrc.z(0);
			current->setExternalForce(tmpfrc);

			// add node force to haptic force, if haptic effect is enabled
			if (m_hapticEnabled)
			{
				force.add(f);
			}

			// compute inner force for travel
			// (point in polygon algorithm)
			if (m_travelMode)
			{
				// check if the cursor is even low enough to contact the travel
				if (penetrationDist < 0)
				{
					if (current->m_pos.y() <= a_cursor.y()) {          // start y <= P.y()
						if (next->m_pos.y() > a_cursor.y())      // an upward crossing
						{
							cVector3d vec1(next->m_pos - current->m_pos);
							vec1.z(0);
							cVector3d vec2(a_cursor - next->m_pos);
							vec2.z(0);
							double val = cCross(vec1, vec2).z();
							if (val > 0)		 // point is left of edge
							{
								++wn;            // have  a valid up intersect
							}
						}
					}
					else {                        // start y > P.y() (no test needed)
						if (next->m_pos.y() <= a_cursor.y())     // a downward crossing
						{
							cVector3d vec1(next->m_pos - current->m_pos);
							vec1.z(0);
							cVector3d vec2(a_cursor - next->m_pos);
							vec2.z(0);
							double val = cCross(vec1, vec2).z();
							if (val < 0)		// P right of  edge
							{
								--wn;            // have  a valid down intersect
							}
						}
					}
				}
			}

			// always set node to align with xy plane
			current->m_pos.z(0);

			//-----------------------------------------------------------------------
			// increment variables for next iteration
			//-----------------------------------------------------------------------

			// increment the link iterator
			if(m_surroundMode || (m_travelMode && m_useTravelLinks))
			{
				iterLink++;
			}

			// store previous iterators for next iterate
			iterPrev = iter;
			prevMeshNode = meshNode;
			if(m_surroundMode || (m_travelMode && m_useTravelLinks))
			{
				iterLinkPrev = iterLink;
			}

			// store previous sphere for next iterate
			prev = current;

			// increment the mesh node
			meshNode++;
		}

		// calculate center of mass
		m_centerOfMass = xyzBar / totalMass;

		// set uniform spacing value
		m_uniformRobotSpacing = m_perimeter / m_robotSurroundSpheres.size();


		// add inner force for travel
		if (m_travelMode)
		{
			// if cursor is inside the travel polygon, then add force
			if (m_hapticEnabled && wn > 0 && penetrationDist < 0)
			{
				force.add(penetrationDist*m_travelStiffness*cVector3d(0,0,-1));
			}
		}

		// set current playdoh force
		m_playdohForce = -1 * force;

		// set current tool force
		m_toolForce = force;
	}


	// store initial transition force
	m_prevTransition = m_transition;

	// account for transitioning
	if (m_transition)
	{
		// limit current computed force
		double transitionForceFactor = m_transitionTimer.getCurrentTimeSeconds() / m_transitionTimeout;
		force.mul(transitionForceFactor);

		// maintain past force
		force.add(cMul((1.0 - transitionForceFactor), m_transitionForce));

		if (m_transitionTimer.timeoutOccurred())
		{
			endTransition();
		}
	}

	// reset spacing timeout
	if (spacingTimeout)
	{
		m_spacingClock.reset();
		m_spacingClock.start();
	}

	// return the haptic force
	return force;
}

//---------------------------------------------------------------------------

void rtsClay::updateDynamics(double a_timeInterval)
{
	// update the deformable world
	m_defWorld->updateDynamics(a_timeInterval);

	rtsClayNode* temp;
	cVector3d pos;
	vector<rtsClayNode*>::iterator iter;
	vector<cVector3d>::iterator iterPos;

	vector<rtsClayNode*> a_robotSpheres;
	if (m_surroundMode)
	{
		// beginning robot position
		iterPos = m_robotPositions.begin();

		a_robotSpheres = m_robotSurroundSpheres;
	}
	else if (m_travelMode)
	{
		// beginning robot position
		iterPos = m_convexHullRobotPositions.begin();

		a_robotSpheres = m_robotTravelSpheres;
	}


	// set robot positions based on dynamics
	int i = 0;
	for (iter = a_robotSpheres.begin(); iter != a_robotSpheres.end(); iter++)
	{
		// dereference iterators
		temp = *iter;
		pos = *iterPos;

		// store new robot position
		if (m_surroundMode)
		{
			m_robotPositions[i] = temp->m_pos;
		}
		else if (m_travelMode)
		{
			int index = m_convexHullMapping[i];
			m_robotPositions[index] = temp->m_pos;
		}

		iterPos++;
		i++;
	}
}

//---------------------------------------------------------------------------

cVector3d rtsClay::computeNodeForce(const cVector3d& a_cursor,
										double a_cursorRadius,
										const cVector3d& a_spherePos,
										double a_radius,
										double a_stiffness)
{

	// compute the reaction forces between the tool and the ith sphere.
	cVector3d force;
	force.zero();
	cVector3d vSphereCursor = a_cursor - a_spherePos;

	// check if both objects are intersecting
	if (vSphereCursor.length() < 0.0000001)
	{
		return (force);
	}

	if (vSphereCursor.length() > (a_cursorRadius + a_radius))
	{
		return (force);
	}

	// compute penetration distance between tool and surface of sphere
	double penetrationDistance = (a_cursorRadius + a_radius) - vSphereCursor.length();
	cVector3d forceDirection = cNormalize(vSphereCursor);
	force = cMul(penetrationDistance * a_stiffness, forceDirection);

	// return result
	return (force);
}

//---------------------------------------------------------------------------

cVector3d rtsClay::computeCylNodeForce(const cVector3d& a_cursor,
	double a_cursorRadius,
	const cVector3d& a_spherePos,
	double a_radius,
	double a_stiffness)
{

	// compute the reaction forces between the tool and the ith sphere.
	cVector3d force;
	force.zero();
	cVector3d vSphereCursor = a_cursor - a_spherePos;

	// project all vectors to the xy plane
	double vHeight = vSphereCursor.z();
	vSphereCursor.z(0);

	// check if both objects are intersecting
	if (vSphereCursor.length() < 0.0000001)
	{
		return (force);
	}

	if (vSphereCursor.length() > (a_cursorRadius + a_radius))
	{
		return (force);
	}

	double cutOffHeight = 1.0;
	if (m_surroundMode)
	{
		cutOffHeight = m_surroundHeight - a_radius;
	}
	else if (m_travelMode)
	{
		cutOffHeight = m_travelHeight - a_radius;
	}


	if (vHeight > cutOffHeight)
	{
		cVector3d a_newSpherePos = a_spherePos;
		a_newSpherePos.z(a_newSpherePos.z() + cutOffHeight);
		return computeNodeForce(a_cursor, a_cursorRadius, a_newSpherePos, a_radius, a_stiffness);
	}

	// compute penetration distance between tool and surface of sphere
	double penetrationDistance = (a_cursorRadius + a_radius) - vSphereCursor.length();
	cVector3d forceDirection = cNormalize(vSphereCursor);
	force = cMul(penetrationDistance * a_stiffness, forceDirection);

	// return result
	return (force);
}

//---------------------------------------------------------------------------

void rtsClay::setBuildingBounds(vector<rtsPolygonBounds> a_buildingBounds)
{
	m_buildingBounds = a_buildingBounds;
}

//---------------------------------------------------------------------------

void rtsClay::setDistributionMode(int a_mode)
{
	// if in standby mode, then initialize to new mode
	if (m_standbyMode)
	{
		if (m_enableMutex)
		{
			m_mutex->lock();
			m_surroundMesh->m_enableMutex = true;
			m_travelMesh->m_enableMutex = true;

			// if change to travel
			if (a_mode == 0)
			{
				// set mode
				setMode(a_mode);

				// initialize travel
				initTravel();
			}

			// if change to surround
			else if (a_mode == 1)
			{
				// set mode
				setMode(a_mode);

				// initialize surround
				initSurround();
			}
			else if (a_mode == 2)
			{
				// set mode
				setMode(a_mode);
			}

			m_mutex->unlock();
			m_enableMutex = false;
		}
	}

	// check changes in distribution modes
	if (a_mode != m_mode || (m_travelMode && m_resetTravel))
	{
		// start transition procedure
		if (a_mode != m_mode)
		{
			startTransition();
		}

		// hide visual and haptics
		if (m_surroundMode)
		{
			m_surroundMesh->setShowEnabled(false, true);
			m_surroundMesh->m_showSkeletonModel = false;
		}
		else if (m_travelMode)
		{
			m_travelMesh->setShowEnabled(false, true);
			m_travelMesh->m_showSkeletonModel = false;
		}

		//setHapticEnabled(false, true);

		// since there was a change in mode, set to standby mode
		setMode(m_defaultMode);
		m_enableMutex = true;
	}
}

//---------------------------------------------------------------------------

void rtsClay::setMutex(std::mutex* a_mutex)
{
	// set mutex
	m_mutex = a_mutex;

	// set mutex in playdoh meshes
	m_surroundMesh->setMutex(m_mutex);
	m_travelMesh->setMutex(m_mutex);

	// initialize enable value
	m_enableMutex = false;
}

//---------------------------------------------------------------------------

bool rtsClay::insideBoundingBox(cVector3d a_pos)
{
	if (a_pos.x() >= m_boundingBoxNeg.x() && a_pos.x() <= m_boundingBoxPos.x()
		&& a_pos.y() >= m_boundingBoxNeg.y() && a_pos.y() <= m_boundingBoxPos.y()
		&& a_pos.z() >= m_boundingBoxNeg.z() && a_pos.z() <= m_boundingBoxPos.z())
	{
		return true;
	}
	else return false;
}

//---------------------------------------------------------------------------

int rtsClay::getNumTravelSpheres()
{
	return m_travelSpheres.size();
}

//---------------------------------------------------------------------------

int rtsClay::getNumSurroundSpheres()
{
	return m_surroundSpheres.size();
}

void rtsClay::setEnableResizeSurround(bool a_resizeSurround) 
{ 
	if (!m_resizeSurround && a_resizeSurround)
	{
		m_resizeStart = true;
	}

	// if done resizing surround, create links again between nodes
	if (m_resizeSurround && !a_resizeSurround)
	{
		// delete the static mesh
		// disable visibility of node model
		m_surroundStaticResizeMesh->m_useSkeletonModel = false;
		m_surroundStaticResizeMesh->m_showSkeletonModel = false;

		// deallocate memory to each node in playdoh mesh
		rtsClayNode* tempNode;
		list<rtsClayNode*>::iterator node;
		for (node = m_surroundStaticResizeMesh->m_nodes.begin(); node != m_surroundStaticResizeMesh->m_nodes.end(); node++)
		{
			// dereference iteratorr
			tempNode = *node;

			// Deallocate memory
			delete tempNode;
			tempNode = NULL;
		}

		// clear the list of nodes in playdoh mesh
		m_surroundStaticResizeMesh->m_nodes.clear();

		// remove the static mesh from the list of meshes in the defermable world
		//m_defWorld->m_gelMeshes.pop_back();

		//setMode(m_defaultMode);
		//setDistributionMode(1);

		//createNodeLinks();

		// show playdoh mesh
		m_surroundMesh->m_useSkeletonModel = true;
		m_surroundMesh->m_showSkeletonModel = true;
		m_surroundMesh->setShowEnabled(m_showEnabled, true);
	}

	m_resizeSurround = a_resizeSurround;
}
//---------------------------------------------------------------------------

void rtsClay::setResizePercentage(double a_resizePercentage)
{
	// set resize percentage
	m_resizePercentage = a_resizePercentage;
}

//---------------------------------------------------------------------------

void rtsClay::setMode(int a_mode)
{
	// reset modes
	m_standbyMode = false;
	m_travelMode = false;
	m_surroundMode = false;

	// set appropriate mode
	if (a_mode == -1)
	{
		m_standbyMode = true;
	}
	else if (a_mode == 0)
	{
		m_travelMode = true;
	}
	else if (a_mode == 1)
	{
		m_surroundMode = true;
	}

	// set member mode
	m_mode = a_mode;
}

//---------------------------------------------------------------------------

void rtsClay::createStraightNodes(vector<cVector3d> a_robotPositions)
{
	int size = a_robotPositions.size();
	for (int i = 0; i < size; i++)
	{
		// define desired vector locations
		unsigned int current = i;
		unsigned int next = (i + 1) % size;

		// set starting and ending positions
		cVector3d p1 = a_robotPositions[current];
		cVector3d p2 = a_robotPositions[next];

		// vector from robot1 to robot2
		cVector3d diff12 = p2 - p1;
		cVector3d norm12 = cNormalize(diff12);
		double distance = diff12.length();

		// define number of spheres
		int numSpheres = (int)(diff12.length() / m_sphereSpacing);

		// add spheres between p1 and p2
		for (int j = 0; j < numSpheres; j++)
		{
			// create sphere and add it to the GELPlaydoh's world
			rtsClayNode* sphere = new rtsClayNode();

			cVector3d result = p1 + (double)j / (double)(numSpheres)*distance*norm12;
			sphere->m_pos = result;

			// initialize entry vector (for building boundary forces)
			sphere->m_entryVec.zero();
			sphere->m_insideBldg = true;

			// set color
			if(m_surroundMode)
				sphere->m_color = m_surroundNodeColor;
			else if (m_travelMode)
			{
				sphere->m_color = m_travelNodeColor;
			}

			// set the first sphere to be at robot location
			if (j == 0)
			{
				sphere->m_atRobot = true;
				//sphere->m_fixed = true;
				//sphere->m_color.set(0.5, 0.0, 0.0, 1.0f);

				// store the indexed location in m_robotPositions for this node
				// and add to the end of the robot spheres list
				if (m_surroundMode)
				{
					sphere->m_IdNum = i;
					m_robotSurroundSpheres.push_back(sphere);
				}
				else if (m_travelMode)
				{
					sphere->m_IdNum = m_convexHullMapping[i];
					m_robotTravelSpheres.push_back(sphere);
				}

			}

			//// set frame size for node
			//sphere->setFrameScale(1.0);

			if (m_surroundMode)
			{
				// add this sphere to the list of surround playdoh spheres
				m_surroundSpheres.push_back(sphere);

				// add this sphere to the list of surround GEL mesh nodes
				m_surroundMesh->m_nodes.push_back(sphere);
			}
			else if (m_travelMode)
			{
				// add this sphere to the list of travel playdoh spheres
				m_travelSpheres.push_back(sphere);

				// add this sphere to the list of travel GEL mesh nodes
				m_travelMesh->m_nodes.push_back(sphere);
			}

		}

	}

	// initialize perimeter
	initPerimeter();

	// enabled haptic effect
	//m_hapticEnabled = true;
}

//---------------------------------------------------------------------------

void rtsClay::initEntryVecs()
{
	// compute center of mass
	cVector3d com(0, 0, 0);
	int numSpheres = m_surroundSpheres.size();
	for (int i = 0; i < numSpheres; i++)
	{
		com.add(m_surroundSpheres[i]->m_pos);
	}
	com.div((double)numSpheres);

	// assign entryVec to be toward the center of mass for each node
	for (int i = 0; i < numSpheres; i++)
	{
		m_surroundSpheres[i]->m_entryVec = cNormalize(com - m_surroundSpheres[i]->m_pos);
	}
}

//---------------------------------------------------------------------------

void rtsClay::createCurvedNodes(vector<cVector3d> a_robotPositions)
{
	// local robot spheres list
	list<rtsClayNode*> a_robotSpheres;

	int size = a_robotPositions.size();
	for (int i = 0; i < size; i++)
	{

		// define desired vector locations
		unsigned int current = i;
		unsigned int next = (i + 1) % size;
		unsigned int previous = (i + size - 2) % size;
		unsigned int future = (i + 2) % size;

		// set previous position
		cVector3d p0 = a_robotPositions[previous];

		// set starting and ending positions
		cVector3d p1 = a_robotPositions[current];
		cVector3d p2 = a_robotPositions[next];

		// set future position
		cVector3d p3 = a_robotPositions[future];

		// vector from robot1 to robot2
		cVector3d diff12 = p2 - p1;
		cVector3d norm12 = cNormalize(diff12);
		double distance = diff12.length();

		// normal vectors for curving
		cVector3d norm01 = p1 - p0;
		norm01.normalize();
		cVector3d norm32 = p2 - p3;
		norm32.normalize();

		// define number of spheres
		int numSpheres = (int)(diff12.length() / m_sphereSpacing);

		// add spheres between p1 and p2
		for (int j = 0; j < numSpheres; j++)
		{
			// create sphere and add it to the GELPlaydoh's world
			rtsClayNode* sphere = new rtsClayNode();
			//m_world->addChild(sphere);

			// set position of sphere along curved path between robot points
			cVector3d curveVec = norm12 + (double)(numSpheres - j) / (double)(numSpheres)*(norm01 + norm32);
			curveVec.normalize();
			//cVector3d curveVec2 = -1*norm12 + (double)(numSpheres - i) / (double)(numSpheres)*norm32;
			//curveVec2.normalize();

			cVector3d result = p1 + (double)j / (double)(numSpheres)*distance*curveVec;
			//cVector3d result2 = a_p2 + (double)i / (double)(numSpheres)*distance*curveVec2;
			sphere->m_pos = result;

			// set color
			if(m_surroundMode)
				sphere->m_color = m_surroundNodeColor;
			else if (m_travelMode)
			{
				sphere->m_color = m_travelNodeColor;
			}

			// set the first sphere to be at robot location
			if (j == 0)
			{
				sphere->m_atRobot = true;
				//sphere->m_fixed = true;
				//sphere->m_color.set(0.5, 0.0, 0.0, 1.0f);

				// store the indexed location in m_robotPositions for this node
				// and add to the end of the robot spheres list
				if (m_surroundMode)
				{
					sphere->m_IdNum = i;
					m_robotSurroundSpheres.push_back(sphere);
				}
				else if (m_travelMode)
				{
					sphere->m_IdNum = m_convexHullMapping[i];
					m_robotTravelSpheres.push_back(sphere);
				}

			}

			// set frame size for node
			sphere->setFrameScale(1.0);

			if (m_surroundMode)
			{
				// add this sphere to the list of surround playdoh spheres
				m_surroundSpheres.push_back(sphere);

				// add this sphere to the list of surround GEL mesh nodes
				m_surroundMesh->m_nodes.push_back(sphere);
			}
			else if (m_travelMode)
			{
				// add this sphere to the list of travel playdoh spheres
				m_travelSpheres.push_back(sphere);

				// add this sphere to the list of travel GEL mesh nodes
				m_travelMesh->m_nodes.push_back(sphere);
			}

		}

	}

	// initialize perimeter
	initPerimeter();

	// enabled haptic effect
	//m_hapticEnabled = true;
}

//---------------------------------------------------------------------------

void rtsClay::createSplinedNodes(vector<cVector3d> a_robotPositions)
{
	int size = a_robotPositions.size();
	for (int i = 0; i < size; i++)
	{

		// define desired vector locations
		unsigned int current = i;
		unsigned int next = (i + 1) % size;
		unsigned int previous = (i + size - 2) % size;
		unsigned int future = (i + 2) % size;

		// set previous position
		cVector3d p0 = a_robotPositions[previous];

		// set starting and ending positions
		cVector3d p1 = a_robotPositions[current];
		cVector3d p2 = a_robotPositions[next];

		// set future position
		cVector3d p3 = a_robotPositions[future];

		// vector from robot1 to robot2
		cVector3d diff12 = p2 - p1;
		cVector3d norm12 = cNormalize(diff12);
		double distance = diff12.length();

		// define number of spheres
		int numSpheres = (int)(diff12.length() / m_sphereSpacing);

		// add spheres between p1 and p2
		for (int j = 0; j < numSpheres; j++)
		{
			// create sphere and add it to the GELPlaydoh's world
			rtsClayNode* sphere = new rtsClayNode();

			// calculate Catmull Rom spline
			double x = (double)i / (double)(numSpheres);
			cVector3d q0 = (x*((2 - x)*x - 1))*p0;
			cVector3d q1 = (x*x*(3*x - 5) + 2)*p1;
			cVector3d q2 = (x*((4 - 3*x) + 1))*p2;
			cVector3d q3 = ((x-1)*x*x)*p3;
			cVector3d q = 0.5 * (q0 + q1 + q2 + q3);

			// set sphere position
			sphere->m_pos = q;

			// set color
			if(m_surroundMode)
				sphere->m_color = m_surroundNodeColor;
			else if (m_travelMode)
			{
				sphere->m_color = m_travelNodeColor;
			}

			// set the first sphere to be at robot location
			if (j == 0)
			{
				sphere->m_atRobot = true;
				//sphere->m_fixed = true;
				//sphere->m_color.set(0.5, 0.0, 0.0, 1.0f);


				if (m_surroundMode)
				{
					sphere->m_IdNum = i;
					m_robotSurroundSpheres.push_back(sphere);
				}
				else if (m_travelMode)
				{
					sphere->m_IdNum = m_convexHullMapping[i];
					m_robotTravelSpheres.push_back(sphere);
				}

			}

			// set frame size for node
			sphere->setFrameScale(1.0);

			if (m_surroundMode)
			{
				// add this sphere to the list of surround playdoh spheres
				m_surroundSpheres.push_back(sphere);

				// add this sphere to the list of surround GEL mesh nodes
				m_surroundMesh->m_nodes.push_back(sphere);
			}
			else if (m_travelMode)
			{
				// add this sphere to the list of travel playdoh spheres
				m_travelSpheres.push_back(sphere);

				// add this sphere to the list of travel GEL mesh nodes
				m_travelMesh->m_nodes.push_back(sphere);
			}

		}

	}

	// initialize perimeter
	initPerimeter();

	// enabled haptic effect
	//m_hapticEnabled = true;
}

//---------------------------------------------------------------------------

void rtsClay::createNodeLinks()
{
	vector<rtsClayNode*> a_spheres;
	rtsClayMesh* a_mesh;
	if (m_surroundMode)
	{
		a_spheres = m_surroundSpheres;
		a_mesh = m_surroundMesh;
	}
	else if (m_travelMode)
	{
		a_spheres = m_travelSpheres;
		a_mesh = m_travelMesh;
	}


	rtsClayNode* current;
	rtsClayNode* next;
	vector<rtsClayNode*>::iterator iter;
	vector<rtsClayNode*>::iterator iterNext;
	for (iter = a_spheres.begin(); iter != a_spheres.end(); iter++)
	{
		// dereference iterator
		current = *iter;

		// get next element in list
		iterNext = iter;
		if ((iterNext != a_spheres.end()) && ((++iterNext) == a_spheres.end()))
		{
			// dereference iterator
			next = *(a_spheres.begin());
		}
		else
		{
			// dereference iterator
			next = *iterNext;
		}

		// create links
		rtsClayLink* newLink = new rtsClayLink(current, next);

		// check if link connects to robot node
		if (current->m_atRobot || next->m_atRobot)
		{
			// make spring flexion zero 
			newLink->m_kSpringFlexion = 0.0;
		}

		// add link to mesh
		a_mesh->m_links.push_back(newLink);
	}
}

void rtsClay::deleteNodeLinks()
{
	if (m_surroundMode)
	{
		// deallocate memory to each link in playdoh mesh
		rtsClayLink* tempLink;
		list<rtsClayLink*>::iterator link;
		for (link = m_surroundMesh->m_links.begin(); link != m_surroundMesh->m_links.end(); link++)
		{
			// dereference iterator
			tempLink = *link;

			// Deallocate memory
			delete tempLink;
			tempLink = NULL;
		}

		// clear the list of links in playdoh mesh
		m_surroundMesh->m_links.clear();

	}
	else if (m_travelMode)
	{
		// deallocate memory to each link in playdoh mesh
		rtsClayLink* tempLink;
		list<rtsClayLink*>::iterator link;
		for (link = m_travelMesh->m_links.begin(); link != m_travelMesh->m_links.end(); link++)
		{
			// dereference iterator
			tempLink = *link;

			// Deallocate memory
			delete tempLink;
			tempLink = NULL;
		}

		// clear the list of links in playdoh mesh
		m_travelMesh->m_links.clear();
	}
}

//---------------------------------------------------------------------------

void rtsClay::initTravelParams()
{
	// compute playdoh radius scale parameter
	double perimeter = 0;
	int numCHRobotPositions = m_convexHullRobotPositions.size();
	for (int i = 0; i < numCHRobotPositions; i++)
	{
		int next;
		if (i < numCHRobotPositions - 1)
		{
			next = i + 1;
		}
		else
		{
			next = 0;
		}
		perimeter += (m_convexHullRobotPositions[next] - m_convexHullRobotPositions[i]).length();
	}

	// set robot perimeter
	m_robotPerimeter = perimeter;

	// calculate workspace radius based on perimeter
	double playdohWorkspaceRadius = perimeter / (2 * C_PI);

	// set default sphere spacing
	m_defaultRadiusToSpacingRatio = 4;

	m_workspaceScale = perimeter;

	// set default sphere radius (based on approx playdoh radius)
	// and clamp to minimum and maximum radius
	m_defaultSphereRadius = cClamp(0.20*playdohWorkspaceRadius, m_toolRadius, 2 * m_toolRadius);

	// set sphere spacing
	m_sphereSpacing = m_defaultSphereRadius / m_defaultRadiusToSpacingRatio;

	// setup default values for nodes
	rtsClayNode::s_default_radius = m_defaultSphereRadius;
	rtsClayNode::s_default_kDampingPos = 10;
	rtsClayNode::s_default_kDampingRot = 0.5;
	rtsClayNode::s_default_muStaticFrictionPos = 0.5;
	rtsClayNode::s_default_muStaticFrictionRot = 0.5;
	rtsClayNode::s_default_muDynamicFrictionPos = 0.2;
	rtsClayNode::s_default_muDynamicFrictionRot = 0.2;
	rtsClayNode::s_default_mass = 0.0001;  // [kg]
	rtsClayNode::s_default_showFrame = false;
	//rtsClayNode::s_default_color.set(1.0, 0.6, 0.6);
	rtsClayNode::s_default_useGravity = true;
	rtsClayNode::s_default_gravity.set(0.00, 0.00, -9.81);
	//rtsClayNode::s_default_stiffness = 0.20 / m_workspaceScale; // [N/m]
	rtsClayNode::s_default_stiffness = 0.4 * (1 / m_defaultRadiusToSpacingRatio) * m_maxNodeStiffness; // [N/m]

	// set max stiffness
	if (rtsClayNode::s_default_stiffness > m_maxNodeStiffness)
	{
		rtsClayNode::s_default_stiffness = m_maxNodeStiffness;
	}

	// set inner travel stiffness
	m_travelStiffness = rtsClayNode::s_default_stiffness * m_defaultRadiusToSpacingRatio;

	// set max travel stiffness
	if (m_travelStiffness > m_maxNodeStiffness)
	{
		m_travelStiffness = m_maxNodeStiffness;
	}

	//std::cout << "travel stiffness: " << rtsClayNode::s_default_stiffness << std::endl;

	// setup default values for links
	rtsClayLink::s_default_kSpringElongation = 10000.0 / m_toolWorkspaceScaleFactor; // [N/m]
	rtsClayLink::s_default_kSpringFlexion = 5.0;   // [Nm/RAD]
	rtsClayLink::s_default_kSpringTorsion = 0.0;   // [Nm/RAD]
	//rtsClayLink::s_default_color = m_travelNodeColor;

	// set travel height
	m_travelHeight = 1.0*m_defaultSphereRadius;
}

//---------------------------------------------------------------------------

void rtsClay::initTravel()
{
	//std::cout << "initTravel called" << std::endl;

	//// delete travel first
	deleteTravel();

	// initialize parameters
	initTravelParams();

	//// add mesh to deformable world meshes
	//m_defWorld->m_gelMeshes.push_back(m_travelMesh);

	// set static friction radius
	m_staticFrictionR = 1.0;

	// set default playdoh stiffness
	m_default_kSpringElongation = rtsClayLink::s_default_kSpringElongation;

	// set playdoh stiffness
	m_kSpringElongation = m_default_kSpringElongation;

	// zero playdoh forces
	m_playdohForce.zero();
	m_prevPlaydohForce.zero();

	// haptic enabled
	m_affectHapticChildren = true;
	//m_hapticEnabled = false;

	// create nodes connecting robot along convex hull
	//createSplinedNodes(m_convexHullRobotPositions);
	vector<cVector3d> positions = m_convexHullRobotPositions;
	positions.pop_back();
	createStraightNodes(positions);

	// create links between each node
	if(m_useTravelLinks)
		createNodeLinks();

	// create remaining nodes for robots inside convex hull??


	// show playdoh mesh
	m_travelMesh->m_useSkeletonModel = true;
	m_travelMesh->m_showSkeletonModel = true;
	m_travelMesh->setShowEnabled(m_showEnabled, true);
}

void rtsClay::initTravel(bool a_show)
{
	//// delete travel first
	deleteTravel();

	//// add mesh to deformable world meshes
	//m_defWorld->m_gelMeshes.push_back(m_travelMesh);

	// set default sphere spacing
	m_defaultRadiusToSpacingRatio = 4;

	// set default sphere radius
	m_defaultSphereRadius = 0.25;

	// set sphere spacing
	m_sphereSpacing = m_defaultSphereRadius / m_defaultRadiusToSpacingRatio;


	// set static friction radius
	m_staticFrictionR = 1.0;


	// set default playdoh stiffness
	m_default_kSpringElongation = rtsClayLink::s_default_kSpringElongation;

	// set playdoh stiffness
	m_kSpringElongation = m_default_kSpringElongation;

	// zero playdoh forces
	m_playdohForce.zero();
	m_prevPlaydohForce.zero();

	// haptic enabled
	m_affectHapticChildren = true;
	//m_hapticEnabled = false;

	// create nodes connecting robot along convex hull
	//createSplinedNodes(m_convexHullRobotPositions);
	vector<cVector3d> positions = m_convexHullRobotPositions;
	positions.pop_back();
	createStraightNodes(positions);

	// create links between each node
	if (m_useTravelLinks)
		createNodeLinks();

	// show playdoh mesh
	m_travelMesh->m_useSkeletonModel = true;
	m_travelMesh->m_showSkeletonModel = a_show;
	m_travelMesh->setShowEnabled(a_show, true);
}

//---------------------------------------------------------------------------

void rtsClay::deleteTravel()
{
	// disable visibility of node model
	m_travelMesh->m_useSkeletonModel = false;
	m_travelMesh->m_showSkeletonModel = false;

	// deallocate memory to each link in playdoh mesh
	rtsClayLink* tempLink;
	list<rtsClayLink*>::iterator link;
	for (link = m_travelMesh->m_links.begin(); link != m_travelMesh->m_links.end(); link++)
	{
		// dereference iterator
		tempLink = *link;

		// Deallocate memory
		delete tempLink;
		tempLink = NULL;
	}

	// clear the list of links in playdoh mesh
	m_travelMesh->m_links.clear();

	// deallocate memory to each node in playdoh mesh
	rtsClayNode* tempNode;
	list<rtsClayNode*>::iterator node;
	for (node = m_travelMesh->m_nodes.begin(); node != m_travelMesh->m_nodes.end(); node++)
	{
		// dereference iterator
		tempNode = *node;

		// Deallocate memory
		delete tempNode;
		tempNode = NULL;
	}

	// clear the list of nodes in playdoh mesh
	m_travelMesh->m_nodes.clear();

	//// remove playdoh mesh from deformable world mesh
	//m_defWorld->m_gelMeshes.clear();

	// clear list of playdoh spheres
	m_travelSpheres.clear();

	// clear list of robot playdoh spheres
	m_robotTravelSpheres.clear();
}

//---------------------------------------------------------------------------

void rtsClay::initSurroundParams()
{
	// compute playdoh radius scale parameter
	double perimeter = 0;
	int numRobotPositions = m_robotPositions.size();
	for (int i = 0; i < numRobotPositions; i++)
	{
		int next;
		if (i < numRobotPositions - 1)
		{
			next = i + 1;
		}
		else
		{
			next = 0;
		}
		perimeter += (m_robotPositions[next] - m_robotPositions[i]).length();
	}

	// set robot perimeter
	m_robotPerimeter = perimeter;

	// calculate workspace radius based on perimeter
	double playdohWorkspaceRadius = perimeter / (2 * C_PI);

	// calculate average spacing
	m_uniformRobotSpacing = perimeter / m_robotPositions.size();

	// initialize spacing factors
	m_spacingFactors.clear();
	for (int i = 0; i < numRobotPositions; i++)
	{
		m_spacingFactors.push_back(1.0);
	}

	// set default sphere spacing
	m_defaultRadiusToSpacingRatio = 4;

	m_workspaceScale = perimeter;

	// set default sphere radius (based on approx playdoh radius)
	// and clamp to minimum and maximum radius
	m_defaultSphereRadius = cClamp(0.20*playdohWorkspaceRadius, m_toolRadius, 2 * m_toolRadius);

	// set sphere spacing
	m_sphereSpacing = m_defaultSphereRadius / m_defaultRadiusToSpacingRatio;

	// setup default values for nodes
	rtsClayNode::s_default_radius = m_defaultSphereRadius;
	rtsClayNode::s_default_kDampingPos = 0.6 * m_workspaceScale; // (hcval)
	rtsClayNode::s_default_kDampingRot = 0.5 * m_workspaceScale;

	// static friction coefficient
	rtsClayNode::s_default_muStaticFrictionPos = 0.10;

	rtsClayNode::s_default_muDynamicFrictionPos = 0.0;
	rtsClayNode::s_default_muStaticFrictionRot = 0.50;
	rtsClayNode::s_default_muDynamicFrictionRot = 0.2;
	rtsClayNode::s_default_mass = 0.002 / m_workspaceScale;  // [kg]
	rtsClayNode::s_default_showFrame = false;
	//rtsClayNode::s_default_color.set(1.0, 0.6, 0.6);
	rtsClayNode::s_default_useGravity = true;
	rtsClayNode::s_default_gravity.set(0.00, 0.00, -9.81*m_toolWorkspaceScaleFactor); // [m / s^2]
	//rtsClayNode::s_default_stiffness = 0.50/m_workspaceScale; // [N/m]
	rtsClayNode::s_default_stiffness = 0.45 * (1 / m_defaultRadiusToSpacingRatio) * m_maxNodeStiffness; // [N/m]
	m_surroundNodeStiffness = rtsClayNode::s_default_stiffness;

	// set max stiffness
	if (rtsClayNode::s_default_stiffness > m_maxNodeStiffness)
	{
		rtsClayNode::s_default_stiffness = m_maxNodeStiffness;
	}

	// setup default values for links
	rtsClayLink::s_default_kSpringElongation = 2000.0 / m_toolWorkspaceScaleFactor; // [N/m] to [N/vir] conversion with factor
	rtsClayLink::s_default_kSpringFlexion = 2.0;   // [Nm/RAD]
	rtsClayLink::s_default_kSpringTorsion = 0.0;   // [Nm/RAD]
	rtsClayLink::s_default_color = m_surroundNodeColor;

	// set surround height	
	m_surroundHeight = 1.0*m_defaultSphereRadius;

	// reset resize percentage
	m_resizePercentage = 1.00;
}

//---------------------------------------------------------------------------

void rtsClay::initSurround()
{
	// delete surround first
	deleteSurround();

	// initialize parameters
	initSurroundParams();

	// add mesh to deformable world meshes
	//m_defWorld->m_gelMeshes.push_back(m_surroundMesh);

	// zero perimeter
	m_perimeter = 0;

	// zero large perimeter
	m_largePerimeter = 0;

	// set static friction radius
	m_staticFrictionR = 1.0;

	// zero geometry perimeter
	m_geomPerimeter = 0;

	// set default playdoh stiffness
	m_default_kSpringElongation = rtsClayLink::s_default_kSpringElongation;

	// set playdoh stiffness
	m_kSpringElongation = m_default_kSpringElongation;

	// zero playdoh forces
	m_playdohForce.zero();
	m_prevPlaydohForce.zero();

	// haptic enabled
	m_affectHapticChildren = true;
	//m_hapticEnabled = false;

	// create nodes connecting robots
	//createSplinedNodes(m_robotPositions);
	createStraightNodes(m_robotPositions);

	// create links between each node
	createNodeLinks();

	// initialize entry vectors in surround mode
	initEntryVecs();

	// show playdoh mesh
	m_surroundMesh->m_useSkeletonModel = true;
	m_surroundMesh->m_showSkeletonModel = true;
	m_surroundMesh->setShowEnabled(m_showEnabled, true);
}

//---------------------------------------------------------------------------

void rtsClay::deleteSurround()
{
	// disable visibility of node model
	m_surroundMesh->m_useSkeletonModel = false;
	m_surroundMesh->m_showSkeletonModel = false;

	// deallocate memory to each link in playdoh mesh
	rtsClayLink* tempLink;
	list<rtsClayLink*>::iterator link;
	for (link = m_surroundMesh->m_links.begin(); link != m_surroundMesh->m_links.end(); link++)
	{
		// dereference iterator
		tempLink = *link;

		// Deallocate memory
		delete tempLink;
		tempLink = NULL;
	}

	// clear the list of links in playdoh mesh
	m_surroundMesh->m_links.clear();

	// deallocate memory to each node in playdoh mesh
	rtsClayNode* tempNode;
	list<rtsClayNode*>::iterator node;
	for (node = m_surroundMesh->m_nodes.begin(); node != m_surroundMesh->m_nodes.end(); node++)
	{
		// dereference iterator
		tempNode = *node;

		// Deallocate memory
		delete tempNode;
		tempNode = NULL;
	}

	// clear the list of nodes in playdoh mesh
	m_surroundMesh->m_nodes.clear();
	
	//// remove playdoh mesh from deformable world mesh
	//m_defWorld->m_gelMeshes.clear();

	// clear list of playdoh spheres
	m_surroundSpheres.clear();

	// clear list of robot playdoh spheres
	m_robotSurroundSpheres.clear();

	// zero perimeter
	m_perimeter = 0;
}

//---------------------------------------------------------------------------

void rtsClay::initPerimeter(){

	// initialize playdoh perimeter
	rtsClayNode* current;
	rtsClayNode* next;
	vector<rtsClayNode*>::iterator iter;
	vector<rtsClayNode*>::iterator iterNext;
	for (iter = m_surroundSpheres.begin(); iter != m_surroundSpheres.end(); iter++)
	{
		// set current sphere
		current = *iter;

		// set next sphere
		iterNext = iter;
		if ((iterNext != m_surroundSpheres.end()) && ((++iterNext) == m_surroundSpheres.end()))
		{
			next = *(m_surroundSpheres.begin());
		}
		else
		{
			next = *iterNext;
		}

		// calculate positions
		cVector3d currentPos = current->m_pos;
		cVector3d nextPos = next->m_pos;
		cVector3d diff = nextPos - currentPos;
		m_perimeter = m_perimeter + diff.length();
	}

	// set geometry perimeter as initial playdoh perimeter
	m_geomPerimeter = m_perimeter;

	// set large perimeter as twice that of the geom perimeter
	m_largePerimeter = 2 * m_geomPerimeter;
}

//---------------------------------------------------------------------------

void rtsClay::resizeSurround()
{
	// delete surround node links
	//deleteNodeLinks();

	//// create static mesh to show original size before resize
	if (m_resizeStart)
	{
		m_surroundStaticResizeMesh->m_useSkeletonModel = true;
		m_surroundStaticResizeMesh->m_showSkeletonModel = true;
		m_surroundStaticResizeMesh->setMutex(m_mutex);
		m_surroundStaticResizeMesh->m_enableMutex = false;
	}

	// object pointers
	rtsClayNode* current;
	rtsClayNode* next;
	rtsClayNode* prev;

	// iterators
	vector<rtsClayNode*>::iterator iter;
	vector<rtsClayNode*>::iterator iterNext;
	vector<rtsClayNode*>::iterator iterPrev;

	// set local spheres and mesh variables
	vector<rtsClayNode*> a_spheres;
	rtsClayMesh* a_mesh;
	if (m_surroundMode)
	{
		a_spheres = m_surroundSpheres;
		a_mesh = m_surroundMesh;
	}
	else if (m_travelMode)
	{
		a_spheres = m_travelSpheres;
		a_mesh = m_travelMesh;
	}

	int robotIndex = 0;
	int numRobots = m_robotPositions.size();
	if (!a_spheres.empty())
	{
		// starting iterator values
		iterPrev = a_spheres.end();
		--iterPrev;
		prev = *iterPrev;

		cVector3d xyzBar;
		xyzBar.zero();
		double totalMass = 0;
		for (iter = a_spheres.begin(); iter != a_spheres.end(); iter++)
		{
			// set current sphere
			current = *iter;

			// set next sphere
			iterNext = iter;
			if ((iterNext != a_spheres.end()) && ((++iterNext) == a_spheres.end()))
			{
				next = *(a_spheres.begin());
			}
			else
			{
				next = *iterNext;
			}

			// set prev sphere
			if (iter == a_spheres.begin())
			{
				prev = a_spheres.back();
			}

			if (m_resizeStart)
			{
				m_surroundStaticResizeMesh->m_showSkeletonModel = m_showEnabled;
				rtsClayNode* sphere = new rtsClayNode();
				sphere->m_pos = current->m_pos;
				sphere->m_radius = current->m_radius;
				sphere->m_fixed = true;
				m_surroundStaticResizeMesh->m_nodes.push_back(sphere);
				sphere->m_color.set(0.0, 0.0, 0.5, 1.0f);
			}

			// calculate mass times x and y
			double mass = current->m_mass;
			xyzBar.x(xyzBar.x() + mass * current->m_pos.x());
			xyzBar.y(xyzBar.y() + mass * current->m_pos.y());
			xyzBar.z(xyzBar.z() + mass * current->m_pos.z());
			totalMass += mass;

			// resize this node from center of mass
			cVector3d pos = current->m_pos - m_centerOfMass;
			pos = m_resizePercentage*pos;
			current->m_pos = pos + m_centerOfMass;
			current->m_pos.z(0); // force the z to zero

			// resize node radius
			//current->m_radius = cClamp(m_resizePercentage * current->m_radius, m_toolRadius, 2 * m_toolRadius);

			// change color
			current->m_color.set(0.1f, 0.0f, 0.9f, 1.0f);

			// update robot positions variable
			if (current->m_atRobot && robotIndex < numRobots)
			{
				m_robotPositions[robotIndex] = current->m_pos;
				robotIndex++;
			}
			
			//-----------------------------------------------------------------------
			// increment variables for next iteration
			//-----------------------------------------------------------------------

			// store previous iterators for next iterate
			iterPrev = iter;

			// store previous sphere for next iterate
			prev = current;
		}
		
		// calculate center of mass (uncomment if center of mass is changing during resize)
		//m_centerOfMass = xyzBar / totalMass;
	}

	if (m_resizeStart)
	{
		m_surroundStaticResizeMesh->setRenderNodes();
		m_resizeStart = false;
	}
}

//---------------------------------------------------------------------------

void rtsClay::updateSpacingFactors(int a_mod1, double a_modFactor)
{
	int N = m_robotSurroundSpheres.size();
	int curIndex = a_mod1;
	int incEnd = N / 2;
	for (int i = 0; i < N; i++)
	{
		// ensure bounds are met
		curIndex = curIndex % N;

		// compute max factor
		double M = N;
		double maxFactor = 2 - a_modFactor;

		// set spacing factor
		m_spacingFactors[curIndex] = computeLinearSpacingFactor(a_modFactor, maxFactor, N / 2, i % (N / 2), (i < incEnd));

		// increment
		curIndex++;
	}
	//cout << "spacingFactors: ";
	//double sum = 0;
	//for (int i = 0; i < N; i++)
	//{
	//	cout << m_spacingFactors[i] << " ";
	//	sum += m_spacingFactors[i];
	//}
	//cout << endl;
	//cout << "spacingFactors sum: " << sum << endl;
}

void rtsClay::resetSpacingFactors()
{
	updateSpacingFactors(0, 1);
}

double rtsClay::computeLinearSpacingFactor(double a_minSpacing, double a_maxSpacing, int a_numRegions, int a_regionNum, bool a_increasing)
{
	if (a_increasing)
		return cClamp(a_minSpacing + (a_maxSpacing - a_minSpacing) / a_numRegions * a_regionNum, a_minSpacing, a_maxSpacing);
	else return cClamp(a_maxSpacing - (a_maxSpacing - a_minSpacing) / a_numRegions * a_regionNum, a_minSpacing, a_maxSpacing);
}

void rtsClay::startTransition()
{
	if (!m_transition)
	{
		m_transition = true;
		m_transitionTimer.reset();
		m_transitionTimer.start();
		m_transitionForce = m_toolForce;
	}
}

void rtsClay::endTransition()
{
	m_transitionTimer.stop();
	m_transition = false;
	m_prevTransition = false;
	m_transitionForce.zero();
}